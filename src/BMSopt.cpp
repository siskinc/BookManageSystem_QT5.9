...
void BMSopt::resetRootPasswd(std::string newRootPasswd)
{
    std::ofstream out("Admin/Root.dat",std::ios_base::out|std::ios_base::binary);
    if(!out){
        std::cout<<"文件Admin/Root.dat打开失败"<<std::endl;
        exit(0);
    }
    out<<newRootPasswd<<std::endl;
    this->m_Rootpasswd = newRootPasswd;
}

void BMSopt::lendBook(std::string bookId, std::string readerId)
{
    Admin * admin  = new Admin;
    admin->lendBook(readerId,this->getReaders(),bookId,this->getBooks());
    this->readersToFile();
    this->booksToFile();
}

void BMSopt::retBook(std::string bookId, std::string readerId)
{
    Admin * admin  = new Admin;
    admin->retBook(readerId,this->getReaders(),bookId,this->getBooks());
    this->readersToFile();
    this->booksToFile();
}
